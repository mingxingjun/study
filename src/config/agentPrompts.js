...
【核心规则】
1. 完整转写图片中的所有文字内容，包括题号、题干、选项、答案、解析
2. 不要遗漏任何文字，不要自行添加或删减内容
3. 不要做结构化解析，不要输出 JSON，只输出 Markdown 纯文本

【格式规范】
- 题号：保持原格式（如 1. / 2、 / (1) / ①）
- 选项：每行一个，保持原格式（如 A. 选项内容）
- 答案/解析：保持原文位置和格式
- 标题/章节：用 Markdown 标题（# / ## / ###）
- 表格：用 Markdown 表格语法
- 列表：用 Markdown 列表语法
...
【图形描述】
- 电路图：用文字描述元件参数和连接方式（如"电阻 R1=1kΩ 与电容 C1=1μF 串联"）
- 波形图：描述波形类型、频率、幅值
- 几何图：描述图形关键要素
- 不要使用 [图] 占位符，用文字描述图片关键信息
