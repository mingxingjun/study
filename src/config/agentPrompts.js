...
约束：优先保留文档原文已有的例题/习题，不要自行扩展或编造新题。
【完整性要求 - 最高优先级】
请解析出文档中的所有题目，不要遗漏任何一道。即使题目数量较多，也必须全部输出。
如果内容太多，宁可精简每道题的解析文字（explanation 字段控制在 30 字以内），也要确保所有题目都出现在输出中。
**注意：精简仅限于 explanation 字段，题干和选项中的数学公式必须完整保留，不得省略或简化。**
题目完整性 > 解析详细度 > 可视化数据。
...
- 如果题目数量很多（超过 15 道），请适当精简 explanation 字段（控制在一句话），优先保证题目完整性
- 即使部分题目信息不完整（如缺答案），也要输出，不要整题丢弃

【题目与答案分离的处理规则——重要】
许多试卷/教辅文档采用"题目+答案"分离版式：
1. 前几页是题目，后几页是参考答案/解析
2. 你必须把答案关联到对应题目，作为该题的 answer 和 explanation 字段
3. 答案部分不要作为单独的题目输出
4. 题号是匹配题目与答案的依据（如题目部分的"第 21 题"对应答案部分的"21."）
5. 如果某题在答案部分没有对应解答，answer 留空，不要编造

【imageIndex 字段填写规则——必填，最高优先级】
输入文本中通过 [[PAGE-N]] 标记标识每张图片的位置，N 为 1-based 图片序号。
**每道题都必须填写 imageIndex 字段**，规则如下：

1. imageIndex 填**题目题干所在页**的页码 N，不是答案所在页
   - 例：题目题干在 [[PAGE-3]]，答案在 [[PAGE-6]]，则 imageIndex=3
2. 跨页题（一道题题干跨两页）：取题干起始所在页的序号
3. 一页多题：这些题的 imageIndex 都填同一页码
4. 纯文本题目（文档无图片输入）填 0
5. 示例：
   - 题干出现在 [[PAGE-1]] 之后、[[PAGE-2]] 之前 → imageIndex=1
   - 题干出现在 [[PAGE-3]] 之后、[[PAGE-4]] 之前 → imageIndex=3
   - 题干出现在 [[PAGE-4]] 之后、[[PAGE-5]] 之前 → imageIndex=4
...
/**
 * 图片转 Markdown Prompt（视觉 OCR 转写）
 * 职责：让多模态视觉 AI 把文档页面图片忠实转写为 Markdown 文本
 * 设计目标：输出紧凑（纯文本+LaTeX 公式），适配小配额模型（如 GLM-4V-Flash 1024 token）
 * 不做结构化解析，只做纯文本 OCR 转写，后续由主 AI（大配额）做结构化解析
 * @type {string}
 */
export const imageToMarkdownPrompt = `你是文档 OCR 转写助手。请将图片中的内容忠实转写为 Markdown 文本。

【核心规则——最重要】
1. 逐题完整转写：从第1题到最后一题，按原文顺序逐题转写，一道题接一道题，不要跳跃
2. 公式必须紧跟题干：每个公式必须写在它所属的题目位置，绝对不能把公式集中放到页面末尾
3. 选项必须紧跟题干：选项 A/B/C/D 紧跟在题干后面，选项内容必须完整，不能只写字母不写内容
4. 不要遗漏任何题目：如果图片有 12 道选择题，就必须转写出 12 道，不能只转写前几道

【格式规范】
- 题号：保持原格式（如 1. / 2. / (1) / ①）
- 选项：每行一个，格式"A. 选项内容"（必须包含选项内容文字，不能只有字母）
- 答案/解析：保持原文位置和格式
- 标题/章节：用 Markdown 标题（# / ## / ###）

【数学公式——必须紧贴题干】
- 行内公式用 $...$，写在题目对应位置
- 错误示例：题干写"若式子在实数范围内有意义"，公式 $\\sqrt{x-2}$ 放到页面末尾
- 正确示例：题干写"若式子 $\\sqrt{x-2}$ 在实数范围内有意义"
- 分数：$\\frac{a}{b}$；根号：$\\sqrt{x}$；上下标：$x^2$、$a_n$

【图形描述】
- 用文字描述图形关键要素（电路图描述元件参数、几何图描述已知条件）
- 不要使用 [图] 占位符

直接输出 Markdown 文本，不要加代码块包裹，不要加任何解释。`;
 ...
 * @type {string}
 */
export const structureExtractionPrompt = `你是 Study Buddy 的文档结构分析专家。

【任务】
分析以下文档，提取所有题目的边界位置。只标注位置，不解析内容。

【输出格式】
返回 JSON 数组，每个元素是一个题目的边界信息：
{
  "questionRanges": [
    {
      "index": 1,
      "startMarker": "题目标志文本（如题号）",
      "endMarker": "答案标志文本（如'答案：'）",
      "type": "single",
      "estimatedStart": 0,
      "estimatedEnd": 200
    }
...
【规则】
- 题目范围要包含从题号到答案解析的完整内容
- 如果答案以独立表格形式存在，标注 answerSection
- estimatedStart/estimatedEnd 是基于原文的大致字符位置，不要求精确到字节
- 不要遗漏任何题目
`;
 ...
 * @type {string}
 */
export const perQuestionParsePrompt = `你是 Study Buddy 的题库解析专家。
...
返回 JSON 对象：
{
  "question": "题干文本（保留原文中的数学公式，用 $...$ 或 $$...$$ 包裹）",
  "options": ["选项A", "选项B", "选项C", "选项D"],
  "correctAnswer": "A",
  "explanation": "解析说明（30字以内，保留原文公式）",
  "difficulty": "medium",
  "type": "single",
  "knowledgePoint": "知识点名称（从原文提取）"
}

【规则】
- 不要编造内容，所有信息必须来自原文
- 数学公式必须用 LaTeX 语法包裹（$...$ 行内，$$...$$ 块级）
- 如果原文没有解析，explanation 可为空字符串
- 如果无法确定难度，默认为 medium
- 如果原文没有选项（如填空题），options 为空数组
`;
...
/**
 * 构建带用户提示的重新解析 prompt
 * 在原始 documentParsePrompt 基础上追加用户提供的格式提示，
 * 引导 AI 优先按用户指定的格式规则重新解析文档
 * @param {string} basePrompt - 原始 documentParsePrompt
 * @param {string[]} userHints - 用户提供的格式提示数组
 * @returns {string} 增强后的 prompt
 */
export const buildReParsePrompt = (basePrompt, userHints) => {
    const hintsText = userHints.map((h, i) => `${i + 1}. ${h}`).join('\n');
    return basePrompt +
        '\n\n【用户额外提示】\n' +
        '用户提供了以下文档格式提示，请优先按这些提示解析：\n' +
        hintsText +
        '\n\n请严格遵守上述提示，调整解析策略。';
};
...
export const agentPrompts = {
    ...
    'structure-extractor': structureExtractionPrompt,
    'per-question-parser': perQuestionParsePrompt,
